...
module parameters
  use constants, only : Rg, amh, mu, pc, yr
  implicit none
#ifdef MPIP
  include "mpif.h"
#endif
  !> Path used to write the output
 character (len=128),parameter ::  outputpath='/datos/esquivel/Guacho-1.2/SN/'
 !> working directory
 character (len=128),parameter ::  workdir='./'
  ...
  real, parameter :: xphys=30.*pc    !< grid extent in X (pohysical units, cgs)
  ...
  real, parameter :: rhosc=amh       !<  mass density scaling
...
!> magnetic fiewld scaling
  ...
  real, parameter :: tmax    = 10.E3*yr/tsc
  !> interval between consecutive outputs
  real, parameter :: dtprint =  1.E3*yr/tsc
