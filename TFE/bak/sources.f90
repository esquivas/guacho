...
!subroutine radpress_source(i,j,k,xc,yc,zc,rc,pp,ppn,s)
...
!subroutine divbcorr_8w_source(i,j,k,pp,1,s)
...
#ifdef TWOFLUID
subroutine twofluid_source(pp, pn,s)
 
  implicit none
  real, intent(in)    :: pp(neq), pp2(neq)
  real, intent(inout) :: s(neq)
  real :: alpha, driftx, drifty, driftz
  !  update the source terms
  alpha = 1E-2
  ...
  !  momenta
  s(2) = s(2) + alpha * pp(1)*pn(1)*driftx
  s(3) = s(3) + alpha * pp(1)*pn(1)*drifty
  s(4) = s(4) + alpha * pp(1)*pn(1)*driftz
  s(5) = s(5) + alpha * pp(1)*pn(2)* (driftx*pp(2)+drifty*pp(3)+driftz*pp(4) )
end subroutine twofluid_source
#endif
...
#ifdef TWOFLUID
  !  twofluid, Stone 1997
  if (twofluid) call twofluid_source(prim,prim2,s)
#endif
