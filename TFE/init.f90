...
#ifdef TWOFLUID
  if (twofluid) then
    allocate (     un(neq,nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
    allocate (    upn(neq,nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
    allocate (primitn(neq,nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
!    allocate (     fn(neq,nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
!    allocate (     gn(neq,nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
!    allocate (     hn(neq,nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
    allocate (Tempn(nxmin:nxmax,nymin:nymax,nzmin:nzmax) )
  end if
#endif
...
!#endif
  ...
  if(rank.eq.master) then
    ...
    if (twofluid) then
      print'(a)', 'TWO FLUID (UNDER CONSTRUCTION)'
      print'(a)', ''
    end if
...
subroutine initflow(itprint)
  ...
  use parameters, only : outputpath, iwarm, twofluid !, itprint0
! check this, if twofluid then think about it (it is not general now)
#ifndef TWOFLUID
  use globals, only : u, rank
#else
  use globals, only : u, un, rank
#endif
  use user_mod, only : initial_conditions, initial_conditionsN
  implicit none
