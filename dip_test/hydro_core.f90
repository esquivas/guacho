...
subroutine calcprim(u,primit, only_ghost)
  ...
  use parameters, only : neq, nxmin, nxmax, nymin, nymax, nzmin, nzmax, &
                         nx, ny, nz, riemann_solver,nxtot,nytot,nztot
  use constants

  use globals, only : Temp, primit0, coords,dx,dy,dz
  use stellarwind, only : rsw
...
!            if((rads<rsw ).and.(Temp(i,k,j) .ne. 1.56e6) )then
!               print*,Temp(i,j,k)
!            endif
