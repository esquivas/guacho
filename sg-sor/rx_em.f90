...
implicit none
  ...
  character (len=128) ::  filehard = 'coef2_8kev.dat'   ! Input File
  integer             :: ip, nd, proc
  real                :: aa, bb
  !initializes MPI
#ifdef MPIP
     ...
     do j=1,ny
        do i=1,nx
          ...
          if( (iobs >=1    ).and.(jobs >=1    ).and. &
            ...
            !  do the interpolation (can be made more general)
...
subroutine  write_RX(itprint,filepath,nxmap,nymap,map)
  ...
  write(file1,'(a,i3.3,a)')  trim(filepath)//'BIN/Rx-',itprint,'.bin'
  unitout=11
...
program Rx_em
  ...
  loop_over_outputs : do itprint=0,300
