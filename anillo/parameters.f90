...
module parameters
  ...
  !>  Enable fluxd-CD cleaning of div B
  ...
  integer, parameter :: cooling = COOL_H  ! originalmente COOL_H
  ...
  integer, parameter :: nxtot=900      !< Total grid size in X
  integer, parameter :: nytot=900      !< Total grid size in Y
  integer, parameter :: nztot=900      !< Total grid size in Z
  ...
  real, parameter :: xphys=(0.06*pc)     !< grid extent in X (physical units, cgs)
  ...
  real, parameter :: tmax    = 12.0 * 558.0 * yr/tsc
  !> interval between consecutive outputs
  real, parameter :: dtprint = 558.0 / 5.0   * yr/tsc
  ...
  integer            :: itprint0=136  !< number of output to do warm start
