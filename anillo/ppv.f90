...
contains
  ...
  !initializes MPI
    ...
    print '(a)' ,'*********************************************'
#endif
    print '(a)', 'Calculating Position Position Velocity data'
  ...
  subroutine  write_ppv(itprint,filepath,nxmap,nymap,nvmap,map)
    ...
    write(file1,'(a,i3.3,a)')  trim(filepath)//'BIN/ppv-',itprint,'.bin'
    unitout=11
...
program ppv
  ...
  integer :: err
  integer :: itprint
  real, parameter :: theta_x = 60.0 *deg
  real, parameter :: theta_y =  0.0 *deg
  real, parameter :: theta_z =  0.0 *deg
  !   map and its dimensions
  integer, parameter :: nvmap=100
  integer            :: nxmap, nymap
  ...
  ! chose output (add option later to input form screen)
  ...
  loop_over_outputs : do itprint=10,10
    ...
    !  write result
    if (rank == master) then
      call write_ppv(itprint,filepath,nxmap,nymap,nvmap,map1)
    end if
