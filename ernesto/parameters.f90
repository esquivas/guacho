...
module parameters
 ...
 character(len=128), parameter :: outputpath='output_test/'
 !> working directory
 character(len=128), parameter :: workdir='./'
  ...
  integer, parameter :: nxtot=150       !< Total grid size in X
  integer, parameter :: nytot=150       !< Total grid size in Y
  integer, parameter :: nztot=150       !< Total grid size in Z
  ...
  real, parameter :: xphys=1.0e+9   !< grid extent in X (physical units, cgs)
  ...
  real, parameter :: tmax=10.05/tsc
    ! interval between consecutive outputs
  real, parameter :: dtprint=.5/tsc
  ...
  integer          :: itprint0=1  !< number of output to do warm start
