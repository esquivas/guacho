...
!  use mpi
  ...
  character (len=128),parameter ::  outputpath='./output/'
  !> working directory
  ...
  integer, parameter :: nytot=300       !< Total grid size in Y
  integer, parameter :: nztot=1         !< Total grid size in Z
  ...
  !  set box size   
  real, parameter :: xmax=0.5         !< grid extent in X (code units)
  real, parameter :: ymax=1.5         !< grid extent in Y (code units)
...
!> magnetic fiewld scaling
  ...
  real, parameter :: tmax    =  50.0
  !> interval between consecutive outputs
  real, parameter :: dtprint = 1.0
