...
#gam=(cv+1.)/cv
gam=1.
vsc=np.sqrt(gam*8.31e7*1e4/1.3)/1e5 #Why 1e5?
tempsc=1e4*gam
psc=1.66e-24*vsc*vsc #amh*mu*vsc^2
...
for nout in range(11,12):
    #  this is the ionization rate
    if (firstrun):
#        Phi = coplot3d(2,nytot/2,0,1,nxtot,nytot,nztot,mpiX,mpiY,mpiZ,nout,nghost=0,path=path,base='ph-')
        temp= coplotTemp(2, nytot/2, neqs,nxtot, nytot, nztot, mpiX,  mpiY, mpiZ, nout, path=path,cv=cv,Tempsc=tempsc, base='points')

        denT= coplot3d(  2, nytot/2, 0,   neqs,  nxtot, nytot, nztot, mpiX, mpiY, mpiZ, nout,path=path, base='points000.')
        vx=   coplot3d(  2, nytot/2, 1,   neqs,  nxtot, nytot, nztot, mpiX, mpiY, mpiZ, nout,path=path, base='points000.')/denT*vsc
        vz=   coplot3d(  2, nytot/2, 3,   neqs,  nxtot, nytot, nztot, mpiX, mpiY, mpiZ, nout,path=path, base='points000.')/denT*vsc
        Pgas= coplot3d(  2, nytot/2, 4,   neqs,  nxtot, nytot, nztot, mpiX, mpiY, mpiZ, nout,path=path, base='points000.')*psc
        denN= coplot3d(  2, nytot/2, 5,   neqs,  nxtot, nytot, nztot, mpiX, mpiY, mpiZ, nout,path=path, base='points000.')
...
#    images=[denT,denN,temp,Pgas,Phi,Psi,yhp,magv,Pram]
    images=[denT,denN,Pgas,yhp,magv,temp]
...
#    maxVs=[0.5e10, 0.5e10, 1e6 , 1e-12, 1e-3, 1e1  , 1., 500 , 1e11]
    ...
    for i in range(0,6):
        ...
        im=grid[i].imshow(images[i],cmap=cmaps[i],origin='lower'
                      ,extent=extent, norm=norms )#, vmin=minVs[i],vmax=maxVs[i] , norm=norms )
