...
run='0.3G-NX800_E_CORR'

path='../BIN/'


for nout in range(30,31):
    ...
    if (firstrun):
        ...
        xhn= coplot3d(2,nytot/2,7,neqs,nxtot,nytot,nztot,mpiX,mpiY,mpiZ,nout,path=path)
        xci= coplot3d(2,nytot/2,8,neqs,nxtot,nytot,nztot,mpiX,mpiY,mpiZ,nout,path=path)
...
#    images=[denT,denN,temp,Pgas,Phi,Psi,xhn,xcn,nh]
    images=[denT,xhn,xcn,nh]

    cmaps=['Blues_r','Blues','Spectral','Spectral','Spectral']
    ...
    F.clf()
    grid = ImageGrid(F, 111,nrows_ncols = (2, 2),direction="row",axes_pad = 0.2, add_all=True,
