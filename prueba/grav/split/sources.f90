...
subroutine divbcorr_8w_source(i,j,k,pp,s,prim0)
  use constants, only : SOLVER_HLLE_SPLIT_ALL, SOLVER_HLLD_SPLIT_ALL
  use parameters, only: riemann_solver
  ...
  ! update source terms
    ...
    if (riemann_solver == SOLVER_HLLE_SPLIT_ALL .or.   &
      ...
      if (present(prim0)) then
        s(2)= s(2)-divB*(pp(6)+prim0(6)) 
        s(3)= s(3)-divB*(pp(7)+prim0(7)) 
        s(4)= s(4)-divB*(pp(8)+prim0(8)) 
      end if
    else
      s(2)= s(2)-divB*pp(6) 
      s(3)= s(3)-divB*pp(7) 
...
subroutine source(i,j,k,prim,prim0,s)

  use user_mod, only : get_user_source_terms
  implicit none
  integer, intent(in)  :: i, j, k
  real, intent(in)     :: prim(neq), prim0(neq)
  ...
  if (user_source_terms) call get_user_source_terms(prim,s,i,j,k,prim0)
