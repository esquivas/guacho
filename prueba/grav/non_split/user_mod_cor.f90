...
subroutine initial_conditions(u)
  ...
  integer :: i, j, k, jj
  real :: Temp_var, mu_var, rho, P_0, P_y, ym, xm, rad, g, y, bb
  ...
  do j = nymin,nymax
     ...
     do k =nzmin, nzmax
        do i = nxmin, nxmax
           ...
           u(1,i,j,k) = (mu_var*P_y/Rg/Temp_var)/rhosc
           if(rad.le.0.5e8) then
             u(5,i,j,k) = cv*0.25*0.12666724448792355/Psc+0.5*(bb/Bsc)**2!cv*4.*0.12666724448792355/Psc+0.5*(bb/Bsc)**2
           else
             u(5,i,j,k) = cv*P_y/Psc+0.5*(bb/Bsc)**2
           end if
        end do
     end do
  end do
