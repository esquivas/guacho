...
!--------------------------------------------------------------------------  
!   if (rank == master) then 
!     allocate (Tempi(1-nghost:nytot+nghost))  ! 15/06/2016
!   end if
!--------------------------------------------------------------------------
#ifdef BFIELD
  if (enable_field_cd) &
...
!#endif
  ...
  if (riemann_solver == SOLVER_HLL) then
    ...
    print'(a)', ''
  else if (riemann_solver == SOLVER_HLL_SPLIT_ALL) then
    print'(a)', 'The Riemann solver is HLL with split in All Variables'
    print'(a)', ''
  else if (riemann_solver == SOLVER_HLLC_SPLIT_ALL) then
    print'(a)', 'The Riemann solver is HLLC with split in All Variables'
    print'(a)', ''  
  ...
  if (eq_of_state == EOS_ADIABATIC) then
    print'(a)', 'The code uses an ADIABATIC EOS'
    print'(a)', ''
