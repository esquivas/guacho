...
module parameters
  ...
  character (len=128),parameter ::  outputpath='./HLLE-Athena/'
  !> working directory
  ...
  integer, parameter :: nxtot=200!256      !< Total grid size in X
  integer, parameter :: nytot=300!256      !< Total grid size in Y
  ...
  real, parameter :: ymax=1.5!1.          !< grid extent in Y (code units)
  real, parameter :: zmax=1./256     !< grid extent in Z (code units)
  ...
  real, parameter :: tmax    = 1.1
  !> interval between consecutive outputs
  real, parameter :: dtprint =  .01 /tsc
