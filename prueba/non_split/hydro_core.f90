...
subroutine prim2f(prim,ff,prim0)
  ...
  use constants, only : SOLVER_HLLE_SPLIT_ALL, SOLVER_HLLD_SPLIT_ALL, SOLVER_HLL_SPLIT_ALL, SOLVER_HLLC_SPLIT_ALL
  implicit none
  real,    dimension(neq), intent(in)  :: prim
  real, dimension(neq), intent(in), optional :: prim0
  ...
  if (mhd) then
    ...
    if (riemann_solver == SOLVER_HLL_SPLIT_ALL .or. &
      ...
      if (present(prim0)) then
      ! HD SPLIT      
        etot= 0.5*(prim(1)+prim0(1))*(prim(2)**2+prim(3)**2+prim(4)**2)+cv*prim(5)
    
        ff(1) = (prim(1)+prim0(1))*prim(2)
        ff(2) = (prim(1)+prim0(1))*prim(2)*prim(2)+prim(5)
        ff(3) = (prim(1)+prim0(1))*prim(2)*prim(3)
        ff(4) = (prim(1)+prim0(1))*prim(2)*prim(4)
        ff(5) = prim(2)*(etot+prim(5)+prim0(5)*(cv+1.))
      end if
    else
    !HD SIN SPLIT
