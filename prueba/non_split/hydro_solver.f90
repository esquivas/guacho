...
subroutine step(dt)
  ...
  use globals, only : up, u, primit,primit0,f, g, h, dx, dy, dz
  use field_cd_module
  use sources
  implicit none
  real :: s(neq)
    ...
    do j=1,ny
      do i=1,nx
        ...
        if (user_source_terms     .or. &
            ...
            eight_wave) then
          call source(i,j,k,primit(:,i,j,k),primit0(:,i,j,k),s)
...
subroutine tstep()
  ...
  if (riemann_solver == SOLVER_HLL_SPLIT_ALL) call hllfluxesSplitAll(1)
  if (riemann_solver == SOLVER_HLLC_SPLIT_ALL) call hllcfluxesSplitAll(1)
  ...
  if (riemann_solver == SOLVER_HLL_SPLIT_ALL) call hllfluxesSplitAll(2)
  if (riemann_solver == SOLVER_HLLC_SPLIT_ALL) call hllcfluxesSplitAll(2)
