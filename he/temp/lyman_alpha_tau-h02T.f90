...
subroutine fill_map(nxmap,nymap,nvmap,vmin,vmax,u,map,dxT,dyT,&
                   theta_x,theta_y,theta_z, itprint)
  ...
  do k=1,nz
     do j=1,ny
        do i=1,nx
          ...
          if( ( (itprint <  71).and.(z >  30.*dz) ) .or.                 &
              ( (itprint >= 71).and.(z < -30.*dz) ) ) then
            ...
            call phigauss(1e6,vzn,vmin,vmax,nvmap,profile)
            ...
            if( (iobs >=1    ).and.(jobs >=1    ).and. &
              ...
              map(iobs,jobs,:)= map(iobs,jobs,:) + &
                                dz*rsc*prim(neqdyn+3)*sigmaLA*lambdaLA*profile(:)
...
subroutine  write_LA(itprint,filepath,nxmap,nymap,nvmap,map)
  ...
  write(file1,'(a,i3.3,a)')  trim(filepath)//'BIN/LA_tau-H0-2T-',itprint,'.bin'
  unitout=11
...
program lyman_alpha_tau
  ...
  loop_over_outputs : do itprint=0,108
    ...
    !  add info to the map
    call fill_map(nxmap,nymap,nvmap,vmin,vmax,u,map,dxT,dyT, theta_x, theta_y, theta_z, itprint)
