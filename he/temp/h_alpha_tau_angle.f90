...
subroutine  write_HA(itprint,i,filepath,nxmap,nymap,nvmap,map)
  ...
  write(file1,'(a,i3.3,a,i3.3,a)')  trim(filepath)//'BIN/HA_tau-',itprint,'_',i,'.bin'
  unitout=11
...
subroutine calc_pop(ne,te,n2p_matrix,n2pp)
  ...
  real,parameter ::  T_low=1e2, T_high=1e7, N_low=1, N_high=1e7
...
program h_alpha_tau
  ...
  loop_over_theta_y : do i=0,nsteps
    ...
    !  write result
    if (rank == master) then
      call write_HA(itprint,i,filepath,nxmap,nymap,nvmap,map1)
    end if
