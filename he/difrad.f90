  ...
  do while( f.ge.fmin  &
    ...
    if (charge_exchange) then
      ...
      if (dtau < 1E-5) then
        ...
        phHot(i,j,k) = phHot(i,j,k)+f*(1.-exp(-dtau) )/((u(neqdyn+3,i,j,k)+1.e-30)*(dx*rsc)**3)
        f=f*exp(-dtau)
        ...
        phCold(i,j,k) = phCold(i,j,k)+f*(1.-exp(-dtau) )/((u(neqdyn+5,i,j,k)+1.e-30)*(dx*rsc)**3 )
        f=f*exp(-dtau)
      end if
    else
      dtau=u(neqdyn+1,i,j,k)*a0*dl*dx*rsc
...
subroutine diffuse_rad()
  ...
  do niter=1, nrays
     ...
     !
     f=S0   !  flujo de fotones ionizantes:not divided by np
  ...
  if (charge_exchange) then
    phCold(:,:,:)=phCold(:,:,:)/real(nmax)
    phHot(:,:,:)=phHot(:,:,:)/real(nmax)
  else
    em=ph
    ph(:,:,:)=ph(:,:,:)/real(nmax)
  end if
