...
!  real, parameter :: sigmaLA = 0.01105, lambdaLA=1.215668e-5 !(c/nu0=lambda)
  real :: alpha, f0, rp, norx, nory, norz, bt, vt, dv, bper, alphaE
  real :: norbx, norby, norbz, coef, cos2xi, sin2xi, cos2xipi, sin2xipi
  real :: bx, by, bz, bxn, byn, bzn
  real :: bxx, byy, bzz, exx, eyy, fluxc, r
  ...
  dv      = sqrt(dz**2+dy**2+dy**2) ! CELL VOLUME
  alphaE  = 90.d0*pi/180.d0 !check why pablo wants to substract 30.32??
...
!                                  dz*rsc*prim(neqdyn+1)*sigmaLA*lambdaLA*profile(:)
                 ...
                 map(iobs,jobs,1)= map(iobs,jobs,1) + &
                      fluxc*coef*f0*cos2xi*bper**(alpha+1.d0)*dv
                 !FLUXQE
                 map(iobs,jobs,2)= map(iobs,jobs,2) + &                
                      fluxc*coef*f0*cos2xipi*bper**(alpha+1.d0)*dv
                 !FLUXU
                 map(iobs,jobs,3)= map(iobs,jobs,3) + &                
                      fluxc*coef*f0*sin2xi*bper**(alpha+1.d0)*dv
                 !FLUXUE
                 map(iobs,jobs,4)= map(iobs,jobs,4) + &                
                      fluxc*coef*f0*sin2xipi*bper**(alpha+1.d0)*dv                 
                 !FLUXI
                 map(iobs,jobs,5)= map(iobs,jobs,5) + &                
                      fluxc*coef*bper**(alpha+1.d0)*dv                 
            end if
          end if
      end do
    end do
...
! map(:,:,1:5) are: fluxQ, fluxQE, FluxU, FluxUE, FluxI
...
subroutine  write_polfrac(itprint,filepath,nxmap,nymap,nvmap,map)
  ...
  write(file1,'(a,i3.3,a)')  trim(filepath)//'BIN/POLFRAC-',itprint,'.bin'
  unitout=11
...
program polfracrad
  ...
  integer, parameter :: nxmap=400, nymap=400,nvmap=5
...
!  filepath='/datos/esquivel/EXO-GUACHO/P1c/'
  filepath='/home/kiara/Guacho3/Guacho/runaway/output/'

  loop_over_outputs : do itprint=1,4
