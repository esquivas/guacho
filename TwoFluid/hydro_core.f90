...
subroutine get_timestep(current_iter, n_iter, current_time, tprint, dt, dump_flag)
  use constants, only : SOLVER_HLLE_SPLIT_ALL, SOLVER_HLLD_SPLIT_ALL
  use parameters, only : nx, ny, nz, cfl, mpi_real_kind, mhd, riemann_solver, twofluid
#ifdef TWOFLUID
  use globals, only : primit, dx, dy, dz, primit0,  primitn0, primitn
  ...
  do k=1,nz
    do j=1,ny
      do i=1,nx
        !  Only compute timestep outside planet
        if (flagP(i,j,k)) then
          cycle
        endif
        if (mhd) then
      ...
      do j=1,ny
        do i=1,nx
          ...
          if (mhd) then
            ...
            if (riemann_solver == SOLVER_HLLE_SPLIT_ALL .or. &
                riemann_solver == SOLVER_HLLD_SPLIT_ALL) then
              call cfast(primitn(5,i,j,k)+primitn0(5,i,j,k),primitn(1,i,j,k)+primitn0(1,i,j,k),&
                   primitn(6,i,j,k)+primitn0(6,i,j,k), primitn(7,i,j,k)+primitn0(7,i,j,k), primitn(8,i,j,k)+primitn0(8,i,j,k), &
                   cx,cy,cz)
            else
              call cfast(primitn(5,i,j,k),primitn(1,i,j,k),&
                   primitn(6,i,j,k), primitn(7,i,j,k), primitn(8,i,j,k), &
                   cx,cy,cz)
            end if

            dtp=min(dtp,dx/(abs(primitn(2,i,j,k))+cx))
            dtp=min(dtp,dy/(abs(primitn(3,i,j,k))+cy))
            dtp=min(dtp,dz/(abs(primitn(4,i,j,k))+cz))
#endif
          else
            ! plain old hydro
            call csound(primitn(5,i,j,k),primitn(1,i,j,k),c)
            dtp=min(dtp,dx/(abs(primitn(2,i,j,k))+c))
            dtp=min(dtp,dy/(abs(primitn(3,i,j,k))+c))
            dtp=min(dtp,dz/(abs(primitn(4,i,j,k))+c))
          end if
