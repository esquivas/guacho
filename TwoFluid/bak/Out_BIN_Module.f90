...
subroutine write_BIN(itprint)
  ...
  ! WRITING NEUTRAL FLUID WHEN TWOFLUID ON!
  ...
  if (twofluid) then
     ...
     write(file2,'(a,i3.3,a,i3.3,a)')  &
          trim(outputpath)//'BIN/pointsN',rank,'.',itprint,'.bin'
     unitout=rank+10
#else
     write(file2,'(a,i3.3,a)')  trim(outputpath)//'BIN/pointsN',itprint,'.bin'
     unitout=10
#endif

     do ip=0, np-1
        if(rank == ip) then
           open(unit=unitout,file=file2,status='replace',access='stream')
           ! write header, then data
           call write_header(unitout,neq,nghost)
           write(unitout) un(:,:,:,:)
        end if
     ...
     close(unitout)
     print'(i3,a,a)',rank," wrote file:",trim(file2)
  end if
#endif
