...
'''
   Returns a 3D array with the Temperature
   ***** Please make sure this coincides with the EOS used***
   This imnplementation assumes that an EOS_H is used and that MHD is off
'''
def get_3D_temperature(file_in, equation, verbose=False):
    head_info = read_header(file_in,verbose=verbose)
    f                   = head_info[0]
    f_kind              = head_info[1]
    nx, ny, nz          = head_info[2]
    ...
    data = np.fromfile(f, dtype=f_kind, \
      ...
      .reshape(neqs,nx+2*nghost,ny+2*nghost,nz+2*nghost,order='F')
    f.close()
    #  The following should be consistent with the u2prim in hydro_core.f90
    #  Scaling in the follwing two lines has been omitted by assuming that rhosc = mh
    n_tot = data[0,nghost:(nx+nghost),nghost:(ny+nghost),nghost:(nz+nghost)] # *rhosc/mh
    n_h0  = data[5,nghost:(nx+nghost),nghost:(ny+nghost),nghost:(nz+nghost)] # *rhosc/mh
    Pgas  = ( data[4,0,nghost:(nx+nghost),nghost:(ny+nghost),nghost:(nz+nghost)]
            - ( data[1,nghost:(nx+nghost),nghost:(ny+nghost),nghost:(nz+nghost)]**2
              + data[2,nghost:(nx+nghost),nghost:(ny+nghost),nghost:(nz+nghost)]**2
              + data[3,nghost:(nx+nghost),nghost:(ny+nghost),nghost:(nz+nghost)]**2 )/n_tot ) * cv
    Tsc = vsc**2/8.3145e7   #
    Temp = P/(2.*ntot-nh0) * Tsc
  return Temp
  ...
  for ip in range(mpi_x) :
    for jp in range(mpi_y) :
      for kp in range(mpi_z) :
        ...
        proc = proc+1
        if (neq == -1) :
            map3d[x0:x0+nx,y0:y0+ny,z0:z0+nz] =get_3D_temperature(file_in, verbose=verbose)
        else :
