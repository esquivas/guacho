...
for (noutp) in range(nout+1):
    ...
    print(lmpData.shape)
    plt.figure(1)
    plt.plot(lmpData[:,1],lmpData[:,2],'o',markersize= 1)
    plt.plot(lmpData[npart,1],lmpData[npart,2],'*',markersize=15, color='green')
    plt.figure(3)
...
plt.legend()
plt.figure(2)
sc = plt.scatter(lmpData[:,1],lmpData[:,2], c=lmpData[:,5],alpha=0.8,cmap='inferno')
cb = plt.colorbar(sc)
